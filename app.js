...
import { getFirestore, initializeFirestore, persistentLocalCache, persistentMultipleTabManager, collection, doc, addDoc, setDoc, getDoc, getDocs, updateDoc, deleteDoc, query, where, onSnapshot, serverTimestamp, writeBatch, arrayUnion, arrayRemove, deleteField } from 'https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js';
...
function writeTripBackup(trips){
  try{
    const safe=trips.map(t=>({id:t.id,name:t.name||'여행',destination:t.destination||'',startDate:t.startDate||'',endDate:t.endDate||'',ownerId:t.ownerId||'',deletedAt:t.deletedAt||null,isDeleted:t.isDeleted===true,restoredAt:t.restoredAt||null,memberIds:Array.isArray(t.memberIds)?t.memberIds:[]}));
...
async function moveTripToTrash(tripId){
  ...
  try{await updateDoc(doc(db,'trips',tripId),{isDeleted:true,deletedAt:serverTimestamp(),deletedBy:user.uid,restoredAt:deleteField(),updatedAt:serverTimestamp(),updatedBy:user.uid})}catch(e){alert(`여행방을 없애지 못했습니다: ${e.message||e}`)}
}
async function restoreTrip(tripId){
  if(!confirm('이 여행방을 복구할까요? 모든 동행자의 홈에 다시 표시됩니다.'))return;
  try{await updateDoc(doc(db,'trips',tripId),{isDeleted:false,deletedAt:deleteField(),deletedBy:deleteField(),restoredAt:serverTimestamp(),updatedAt:serverTimestamp(),updatedBy:user.uid})}catch(e){alert(`여행방을 복구하지 못했습니다: ${e.message||e}`)}
}
async function permanentlyDeleteTrip(tripId){
  const snap=await getDoc(doc(db,'trips',tripId));
  if(!snap.exists())return;
  const trip={id:snap.id,...snap.data()};
  if(trip.ownerId!==user.uid)return alert('방장만 영구 삭제할 수 있습니다.');
  if(prompt(`"${trip.name||'여행'}"의 모든 일정·지출·장소·메모를 완전히 삭제합니다. 계속하려면 영구삭제를 입력하세요.`)!=='영구삭제')return;
  try{
    const subcollections=['itinerary','flights','stays','expenses','exchanges','commonFunds','packing','memos','places'],refs=[];
    for(const name of subcollections){const items=await getDocs(collection(db,'trips',tripId,name));items.docs.forEach(item=>refs.push(item.ref))}
    ...
    const finalBatch=writeBatch(db),inviteRef=trip.inviteCode?doc(db,'invites',trip.inviteCode):null;
    if(inviteRef){const invite=await getDoc(inviteRef);if(invite.exists())finalBatch.delete(inviteRef)}
    finalBatch.delete(doc(db,'trips',tripId));await finalBatch.commit();
    alert('여행방과 저장된 데이터가 완전히 삭제되었습니다.');
  }catch(e){alert(`영구 삭제하지 못했습니다: ${e.message||e}`)}
}
...
function listenSubs(){['itinerary','flights','stays','expenses','exchanges','commonFunds','packing','memos','places'].forEach(name=>{subUnsubs.push(onSnapshot(collection(db,'trips',currentTrip.id,name),s=>{cache[name]=s.docs.map(d=>({id:d.id,...d.data()}));renderAll()}))})}
...
async function googlePlaceSearchForm(){
  ...
  modal(`<h3>Google 장소 검색</h3><div class="field"><label>가게 또는 장소</label><div class="row"><input id="gmQuery" placeholder="예: 타이베이 딘타이펑"><button id="gmSearchGo" class="btn primary">검색</button></div></div><p id="gmSearchMsg" class="note">검색 결과를 선택하면 이 여행방의 추천 장소에 저장됩니다.</p><div class="actions"><button id="gmOpenMapSearch" class="btn">Google 지도 화면에서 검색</button></div><div id="gmResults" class="google-results"></div><div class="row"><button class="btn" data-close="1">닫기</button></div>`);
  $('#gmQuery').onkeydown=e=>{if(e.key==='Enter'){$('#gmSearchGo').click();e.preventDefault()}};
  $('#gmOpenMapSearch').onclick=()=>{const query=$('#gmQuery').value.trim()||currentTrip.destination||'';window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(query)}`,'_blank','noopener')};
...
const weatherIcon=code=>code===0?'☀️':code<=3?'⛅':code<=48?'🌫️':code<=67?'🌧️':code<=77?'🌨️':code<=82?'🌦️':code<=99?'⛈️':'·';
const addIsoDays=(iso,amount)=>{const d=new Date(iso+'T00:00:00');d.setDate(d.getDate()+amount);return d.toISOString().slice(0,10)};
async function fetchWeatherLocation(queryText){
  const geocode=await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(queryText)}&count=1&language=ko&format=json`).then(r=>{if(!r.ok)throw Error(`${queryText} 위치 검색에 실패했습니다.`);return r.json()});
  const loc=geocode.results?.[0];if(!loc)throw Error(`${queryText} 위치를 찾지 못했습니다.`);
  const forecast=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${loc.latitude}&longitude=${loc.longitude}&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max&timezone=auto&forecast_days=16&past_days=3`).then(r=>{if(!r.ok)throw Error(`${queryText} 날씨를 불러오지 못했습니다.`);return r.json()});
  const byDate=new Map((forecast.daily?.time||[]).map((date,index)=>[date,{code:forecast.daily.weather_code?.[index],max:forecast.daily.temperature_2m_max?.[index],min:forecast.daily.temperature_2m_min?.[index],rain:forecast.daily.precipitation_probability_max?.[index]}]));
  return {query:queryText,label:`${loc.name}${loc.country?', '+loc.country:''}`,byDate,dates:forecast.daily?.time||[]};
}
function weatherCell(item){
  if(!item)return '<td class="weather-missing">아직<br>미공개</td>';
  return `<td><span class="weather-symbol">${weatherIcon(item.code)}</span><b>${Math.round(item.min)}° / ${Math.round(item.max)}°</b><small>강수 ${item.rain??'-'}%</small></td>`;
}
async function loadWeather(){
  ...
  try{
    ...
    const defaultLocations=[currentTrip.destination,...tripCountries(currentTrip).map(x=>x.name)].map(x=>(x||'').trim()).filter(Boolean),hiddenLocations=(currentTrip.hiddenWeatherLocations||[]).map(x=>(x||'').trim()).filter(Boolean),customLocations=(currentTrip.weatherLocations||[]).map(x=>(x||'').trim()).filter(Boolean),locationNames=[...defaultLocations,...customLocations].filter((x,i,a)=>x&&a.indexOf(x)===i&&!hiddenLocations.includes(x));
    ...
    const body=results.map(loc=>`<tr><th class="weather-location"><span>${esc(loc.label)}</span>${`<button class="btn tiny danger-btn" data-weather-remove="${esc(loc.query)}">삭제</button>`}${loc.error?`<small>${esc(loc.error)}</small>`:''}</th>${targetDates.map(date=>weatherCell(loc.byDate.get(date))).join('')}</tr>`).join('');
...
async function registerWeatherLocation(locationName){
  ...
  const existing=currentTrip.weatherLocations||[],hidden=currentTrip.hiddenWeatherLocations||[];if(existing.includes(name)&&!hidden.includes(name))return alert('이미 날씨표에 등록된 장소입니다.');
  await updateDoc(doc(db,'trips',currentTrip.id),{weatherLocations:arrayUnion(name),hiddenWeatherLocations:arrayRemove(name),updatedAt:serverTimestamp(),updatedBy:user.uid});
  currentTrip.weatherLocations=[...existing,name];closeModal();await loadWeather();
}
async function removeWeatherLocation(locationName){
  if(!confirm(`${locationName} 날씨를 표에서 삭제할까요? 나중에 검색해서 다시 등록할 수 있습니다.`))return;
  await updateDoc(doc(db,'trips',currentTrip.id),{weatherLocations:arrayRemove(locationName),hiddenWeatherLocations:arrayUnion(locationName),updatedAt:serverTimestamp(),updatedBy:user.uid});
  currentTrip.weatherLocations=(currentTrip.weatherLocations||[]).filter(x=>x!==locationName);currentTrip.hiddenWeatherLocations=[...new Set([...(currentTrip.hiddenWeatherLocations||[]),locationName])];await loadWeather();
}
...
function flightForm(){modal(`<h3>항공편 추가</h3><div class="row"><div class="field"><label>항공사</label><input id="flAir"></div><div class="field"><label>편명</label><input id="flNo"></div></div><div class="field"><label>날짜</label><input id="flDate" type="date"></div><div class="row"><div class="field"><label>출발지</label><input id="flFrom"></div><div class="field"><label>출발시간</label><input id="flDepart" type="time"></div></div><div class="row"><div class="field"><label>도착지</label><input id="flTo"></div><div class="field"><label>도착시간</label><input id="flArrive" type="time"></div></div><div class="field"><label>예약번호</label><input id="flBook"></div><div class="field"><label>메모사항</label><textarea id="flNote" placeholder="수하물, 좌석, 탑승 주의사항 등"></textarea></div><div class="row"><button class="btn" data-close="1">취소</button><button id="flSave" class="btn primary">저장</button></div>`);setTimeout(()=>$('#flSave').onclick=async()=>{await addDoc(collection(db,'trips',currentTrip.id,'flights'),{airline:$('#flAir').value,flightNo:$('#flNo').value,date:$('#flDate').value,from:$('#flFrom').value,to:$('#flTo').value,depart:$('#flDepart').value,arrive:$('#flArrive').value,booking:$('#flBook').value,note:$('#flNote').value.trim(),createdAt:serverTimestamp()});closeModal()},0)}

function renderStays(){const a=cache.stays||[];$('#panel-stays').innerHTML=`<div class="section-title"><h2>숙소</h2><button id="addStay" class="btn primary">+ 숙소 추가</button></div><div class="list">${a.length?a.map(x=>`<div class="item"><div><h4>${esc(x.name)}</h4><div class="sub">${esc(x.checkin||'')} → ${esc(x.checkout||'')} · ${esc(x.address||'')}</div><div class="sub">예약번호 ${esc(x.booking||'-')}</div>${x.note?`<div class="record-note">${esc(x.note)}</div>`:''}</div><div class="actions"><button class="btn small" data-edit="stays:${x.id}">수정</button><button class="btn small" data-del="stays:${x.id}">삭제</button></div></div>`).join(''):'<div class="empty">숙소 예약 정보를 저장하세요.</div>'}</div>`;$('#addStay').onclick=stayForm;bindDeletes()}
function stayForm(){modal(`<h3>숙소 추가</h3><div class="field"><label>숙소명</label><input id="stName"></div><div class="row"><div class="field"><label>체크인</label><input id="stIn" type="date"></div><div class="field"><label>체크아웃</label><input id="stOut" type="date"></div></div><div class="field"><label>주소</label><input id="stAddr"></div><div class="field"><label>예약번호</label><input id="stBook"></div><div class="field"><label>메모사항</label><textarea id="stNote" placeholder="체크인 방법, 조식, 짐 보관 등"></textarea></div><div class="row"><button class="btn" data-close="1">취소</button><button id="stSave" class="btn primary">저장</button></div>`);setTimeout(()=>$('#stSave').onclick=async()=>{await addDoc(collection(db,'trips',currentTrip.id,'stays'),{name:$('#stName').value,checkin:$('#stIn').value,checkout:$('#stOut').value,address:$('#stAddr').value,booking:$('#stBook').value,note:$('#stNote').value.trim(),createdAt:serverTimestamp()});closeModal()},0)}


function commonFundStats(){
  const contributions=cache.commonFunds||[],expenses=cache.expenses||[];
  const deposits=contributions.reduce((n,x)=>n+Number(x.baseAmount||0),0);
  const spent=expenses.filter(x=>x.fundType==='common-cash'||x.fundType==='common-card'||x.method==='공동 현금'||x.method==='공동 카드').reduce((n,x)=>n+Number(x.baseAmount||0),0);
  return {contributions,deposits,spent,balance:deposits-spent};
}
function settlementLedger(){
  const memberIds=Object.keys(currentTrip.members||{}),fundUid='__common_fund__',bal=Object.fromEntries([...memberIds,fundUid].map(uid=>[uid,0]));
  for(const entry of cache.commonFunds||[])if(bal.hasOwnProperty(entry.memberUid)){const amount=Number(entry.baseAmount||0);bal[entry.memberUid]+=amount;bal[fundUid]-=amount}
  for(const expense of cache.expenses||[]){
    const amount=Number(expense.baseAmount||0),fromFund=expense.fundType==='common-cash'||expense.fundType==='common-card'||expense.method==='공동 현금'||expense.method==='공동 카드';
    const payer=fromFund?fundUid:expense.payerUid;if(bal.hasOwnProperty(payer))bal[payer]+=amount;
    if(expense.type==='personal'){const owner=expense.personalUid||expense.payerUid;if(bal.hasOwnProperty(owner))bal[owner]-=amount}
    else{const participants=(expense.participantUids?.length?expense.participantUids:memberIds).filter(uid=>bal.hasOwnProperty(uid));const share=participants.length?amount/participants.length:0;participants.forEach(uid=>bal[uid]-=share)}
  }
  const debt=Object.entries(bal).filter(([,v])=>v<-.01).map(([u,v])=>[u,-v]),credit=Object.entries(bal).filter(([,v])=>v>.01).map(([u,v])=>[u,v]),out=[];let i=0,j=0;
  while(i<debt.length&&j<credit.length){const amount=Math.min(debt[i][1],credit[j][1]);out.push({from:debt[i][0],to:credit[j][0],amount});debt[i][1]-=amount;credit[j][1]-=amount;if(debt[i][1]<.01)i++;if(credit[j][1]<.01)j++}
  return {balances:bal,transfers:out};
}
function commonFundForm(){
  const currencies=[currentTrip.baseCurrency,...tripForeignCurrencies(currentTrip)];
  modal('<h3>공동 자금 입금</h3><div class="field"><label>입금한 사람</label><select id="fundMember">'+memberOptions(user.uid)+'</select></div><div class="row"><div class="field"><label>금액</label><input id="fundAmount" type="number" min="0" step="any"></div><div class="field"><label>통화</label><select id="fundCurrency">'+limitedCurrencyOptions(currencies[0])+'</select></div></div><div class="field"><label>공동 자금 종류</label><select id="fundType"><option value="common-cash">공동 현금</option><option value="common-card">공동 카드</option></select></div><div class="field"><label>입금일</label><input id="fundDate" type="date" value="'+new Date().toISOString().slice(0,10)+'"></div><div class="field"><label>메모</label><input id="fundNote" placeholder="예: 여행 전 공동경비"></div><div class="row"><button class="btn" data-close="1">취소</button><button id="fundSave" class="btn primary">입금 저장</button></div><p id="fundMsg" class="note"></p>');
  $('#fundSave').onclick=async()=>{const button=$('#fundSave'),message=$('#fundMsg');try{const amount=Number($('#fundAmount').value),currency=$('#fundCurrency').value;if(!(amount>0))throw Error('0보다 큰 금액을 입력해 주세요.');button.disabled=true;const converted=await fxToBase(currency,amount);await addDoc(collection(db,'trips',currentTrip.id,'commonFunds'),{memberUid:$('#fundMember').value,originalAmount:amount,currency,baseAmount:converted.base,fxRate:converted.rate,fxSource:converted.source,fundType:$('#fundType').value,date:$('#fundDate').value,note:$('#fundNote').value.trim(),createdBy:user.uid,createdAt:serverTimestamp()});closeModal()}catch(e){message.textContent=e.message||String(e);button.disabled=false}};
}
...
function replacementRenderMoney(){
  const a=cache.expenses||[],fund=commonFundStats(),ledger=settlementLedger(),shared=a.filter(x=>x.type==='shared').reduce((n,x)=>n+Number(x.baseAmount||0),0),personal=a.filter(x=>x.type==='personal').reduce((n,x)=>n+Number(x.baseAmount||0),0),today=new Date().toISOString().slice(0,10);
  const name=uid=>uid==='__common_fund__'?'공동 자금':(currentTrip.members?.[uid]?.nickname||'동행자');
  const people=expenseBreakdown().map(row=>{const deposits=fund.contributions.filter(x=>x.memberUid===row.uid).reduce((n,x)=>n+Number(x.baseAmount||0),0),todaySpent=a.filter(x=>(x.date||'')===today&&(x.personalUid===row.uid||x.payerUid===row.uid)).reduce((n,x)=>n+Number(x.baseAmount||0),0),balance=Number(ledger.balances[row.uid]||0);return `<details class="person-expense"><summary><span><b>${esc(row.name)}</b><small>개인 ${money(row.personal,currentTrip.baseCurrency)} · 공동 분담 ${money(row.shared,currentTrip.baseCurrency)}</small></span><strong class="${balance<0?'danger':'good'}">${balance<0?'추가 정산 '+money(-balance,currentTrip.baseCurrency):'받을 금액 '+money(balance,currentTrip.baseCurrency)}</strong></summary><div class="person-expense-detail"><div><span>공동 자금 입금</span><b>${money(deposits,currentTrip.baseCurrency)}</b></div><div><span>오늘 사용액</span><b>${money(todaySpent,currentTrip.baseCurrency)}</b></div><div><span>개인+공동 부담</span><b>${money(row.total,currentTrip.baseCurrency)}</b></div></div></details>`}).join('');
  const contributions=fund.contributions.length?fund.contributions.map(x=>`<div class="item"><div><h4>${esc(name(x.memberUid))} · ${x.fundType==='common-card'?'공동 카드':'공동 현금'}</h4><div class="sub">${esc(x.date||'날짜 없음')} · ${money(x.originalAmount,x.currency||currentTrip.baseCurrency)}${x.note?` · ${esc(x.note)}`:''}</div></div><div class="actions"><span class="money">${money(x.baseAmount,currentTrip.baseCurrency)}</span><button class="btn small" data-delete-fund="${x.id}">삭제</button></div></div>`).join(''):'<div class="empty">공동 자금 입금 내역이 없습니다.</div>';
  $('#panel-money').innerHTML=`<div class="section-title"><h2>예산·지출 관리</h2><div class="actions"><button id="addFund" class="btn">+ 공동 자금 입금</button><button id="addExpense" class="btn primary">+ 지출 추가</button></div></div><div class="kpis compact"><div class="kpi"><div class="l">공동 자금 입금</div><div class="n">${money(fund.deposits,currentTrip.baseCurrency)}</div></div><div class="kpi"><div class="l">공동 자금 지출</div><div class="n">${money(fund.spent,currentTrip.baseCurrency)}</div></div><div class="kpi"><div class="l">현재 공동 자금</div><div class="n ${fund.balance<0?'danger':'good'}">${money(fund.balance,currentTrip.baseCurrency)}</div></div><div class="kpi"><div class="l">개인 지출</div><div class="n">${money(personal,currentTrip.baseCurrency)}</div></div></div><p class="note">공동 자금보다 큰 금액도 기록할 수 있습니다. 부족액은 최종 정산에 자동 반영되며, 입금액은 비율로 다시 나누지 않습니다.</p><div class="section-title"><h2>인원별 정산</h2></div><div class="card">${people}</div><div class="section-title"><h2>공동 자금 입금 내역</h2></div><div class="list">${contributions}</div><div class="section-title"><h2>결제 내역</h2></div><div class="list">${a.length?a.map(e=>`<div class="item"><div><h4>${esc(e.title)}</h4><div class="sub">${esc(e.date||'날짜 없음')} · ${e.type==='shared'?'공동비용':'개인비용'} · ${e.fundType==='common-cash'||e.fundType==='common-card'||e.method==='공동 현금'||e.method==='공동 카드'?'공동 자금 결제':`결제 ${esc(name(e.payerUid))}`}</div><div class="sub">${money(e.originalAmount,e.currency||currentTrip.baseCurrency)} → ${money(e.baseAmount,currentTrip.baseCurrency)}</div></div><div class="right"><div class="money">${money(e.baseAmount,currentTrip.baseCurrency)}</div><div class="actions"><button class="btn small" data-edit="expenses:${e.id}">수정</button><button class="btn small" data-del="expenses:${e.id}">삭제</button></div></div></div>`).join(''):'<div class="empty">지출을 기록해 주세요.</div>'}</div><div class="section-title"><h2>최종 정산</h2></div><div class="card">${ledger.transfers.length?ledger.transfers.map(x=>`<div class="item"><b>${esc(name(x.from))} → ${esc(name(x.to))}</b><span class="money">${money(x.amount,currentTrip.baseCurrency)}</span></div>`).join(''):'현재 서로 보낼 돈이 없습니다.'}</div>`;
  $('#addExpense').onclick=expenseForm;$('#addFund').onclick=commonFundForm;$('[data-delete-fund]').forEach(b=>b.onclick=async()=>{if(confirm('이 공동 자금 입금을 삭제할까요?'))await deleteDoc(doc(db,'trips',currentTrip.id,'commonFunds',b.dataset.deleteFund))});bindDeletes();
}
...
function replacementExpenseForm(){
  const currencies=[currentTrip.baseCurrency,...tripForeignCurrencies(currentTrip)],initial=currencies[0];
  modal(`<h3>지출 추가</h3><div class="field"><label>내용</label><input id="exTitle" placeholder="저녁 식사"></div><div class="row"><div class="field"><label>금액</label><input id="exAmt" type="number" min="0" step="any"></div><div class="field"><label>결제 통화</label><select id="exCur">${limitedCurrencyOptions(initial)}</select></div></div><div id="manualRateWrap" class="field"><label>수동 환율 (선택)</label><input id="exManualRate" type="number" min="0" step="any"></div><div class="field"><label>지출일</label><input id="exDate" type="date" value="${new Date().toISOString().slice(0,10)}"></div><div class="row"><div class="field"><label>비용 유형</label><select id="exType"><option value="shared">공동비용</option><option value="personal">개인비용</option></select></div><div class="field"><label>결제 자금</label><select id="exFund"><option value="personal">개인 돈</option><option value="common-cash">공동 현금</option><option value="common-card">공동 카드</option></select></div></div><div id="payerWrap" class="field"><label>실제 결제자</label><select id="exPayer">${memberOptions(user.uid)}</select></div><div id="personalWrap" class="field hidden"><label>개인비용 사용자</label><select id="exPersonal">${memberOptions(user.uid)}</select></div><div class="row"><button class="btn" data-close="1">취소</button><button id="exSave" class="btn primary">저장</button></div><p id="exMsg" class="note"></p>`);
  const update=()=>{const cur=$('#exCur').value;$('#manualRateWrap').classList.toggle('hidden',cur===currentTrip.baseCurrency);$('#personalWrap').classList.toggle('hidden',$('#exType').value!=='personal');$('#payerWrap').classList.toggle('hidden',$('#exFund').value!=='personal')};
  update();$('#exCur').onchange=update;$('#exType').onchange=update;$('#exFund').onchange=update;
  $('#exSave').onclick=async()=>{const button=$('#exSave'),message=$('#exMsg');try{const title=$('#exTitle').value.trim(),amount=Number($('#exAmt').value),cur=$('#exCur').value;if(!title)throw Error('지출 내용을 입력해 주세요.');if(!(amount>0))throw Error('0보다 큰 금액을 입력해 주세요.');button.disabled=true;const res=await fxToBase(cur,amount,Number($('#exManualRate').value||0)),fundType=$('#exFund').value;await addDoc(collection(db,'trips',currentTrip.id,'expenses'),{title,date:$('#exDate').value,originalAmount:amount,currency:cur,baseAmount:res.base,fxRate:res.rate,fxSource:res.source,type:$('#exType').value,fundType,payerUid:fundType==='personal'?$('#exPayer').value:null,personalUid:$('#exType').value==='personal'?$('#exPersonal').value:null,participantUids:Object.keys(currentTrip.members||{}),method:fundType==='common-cash'?'공동 현금':fundType==='common-card'?'공동 카드':'개인 결제',createdBy:user.uid,createdAt:serverTimestamp()});closeModal()}catch(e){message.textContent=e.message||String(e);button.disabled=false}};
}
...
function packingAssigneeChoices(selectedUids=[]){
  return Object.entries(currentTrip.members||{}).map(([uid,m])=>`<label class="member-choice"><input type="checkbox" name="packAssignee" value="${uid}" ${selectedUids.includes(uid)?'checked':''}><span>${esc(m.nickname||m.email)}</span></label>`).join('');
}
function packingForm(){
  modal(`<h3>준비물 추가</h3><div class="field"><label>준비물</label><input id="packText" placeholder="여권, 충전기 등"></div><div class="field"><label>챙길 사람 (선택·복수 가능)</label><div class="member-choice-grid">${packingAssigneeChoices()}</div><p class="note">선택하지 않으면 모두가 함께 확인하는 공용 준비물이 됩니다. 여러 명을 선택하면 사람별로 완료 체크할 수 있습니다.</p></div><div class="row"><button class="btn" data-close="1">취소</button><button id="packSave" class="btn primary">저장</button></div>`);
  $('#packSave').onclick=async()=>{const text=$('#packText').value.trim();if(!text)return alert('준비물을 입력해 주세요.');const assigneeUids=$$$('input[name="packAssignee"]:checked').map(x=>x.value);await addDoc(collection(db,'trips',currentTrip.id,'packing'),{text,assigneeUids,completedByUids:[],done:false,createdBy:user.uid,createdAt:serverTimestamp()});closeModal()};
}
function renderPacking(){
  const a=cache.packing||[];
  $('#panel-packing').innerHTML=`<div class="section-title"><h2>준비물 체크리스트</h2><button id="addPack" class="btn primary">+ 준비물</button></div><div class="list">${a.length?a.map(x=>{const assignees=Array.isArray(x.assigneeUids)?x.assigneeUids:[],completed=Array.isArray(x.completedByUids)?x.completedByUids:[],allDone=assignees.length?assignees.every(uid=>completed.includes(uid)):!!x.done;const checks=assignees.length?`<div class="packing-people">${assignees.map(uid=>`<label class="check person-check"><input type="checkbox" data-pack-person="${x.id}:${uid}" ${completed.includes(uid)?'checked':''}><span>${esc(currentTrip.members?.[uid]?.nickname||'동행자')}</span></label>`).join('')}</div>`:`<label class="check"><input type="checkbox" data-check="${x.id}" ${x.done?'checked':''}><span>공용 준비 완료</span></label>`;return `<div class="item packing-item"><div><h4 class="${allDone?'done-text':''}">${esc(x.text)}</h4>${checks}</div><div class="actions"><button class="btn small" data-edit="packing:${x.id}">수정</button><button class="btn small" data-del="packing:${x.id}">삭제</button></div></div>`}).join(''):'<div class="empty">여권, eSIM, 상비약 등 함께 체크하세요.</div>'}</div>`;
  $('#addPack').onclick=packingForm;
  $$('[data-check]').forEach(c=>c.onchange=()=>updateDoc(doc(db,'trips',currentTrip.id,'packing',c.dataset.check),{done:c.checked,doneBy:c.checked?user.uid:null}));
  $$('[data-pack-person]').forEach(c=>c.onchange=()=>{const [id,uid]=c.dataset.packPerson.split(':');return updateDoc(doc(db,'trips',currentTrip.id,'packing',id),{completedByUids:c.checked?arrayUnion(uid):arrayRemove(uid)})});
  bindDeletes();
}
function memoForm(){
  modal(`<h3>메모 추가</h3><div class="field"><label>제목</label><input id="meTitle"></div><div class="field"><label>내용</label><textarea id="meText"></textarea></div><div class="row"><button class="btn" data-close="1">취소</button><button id="meSave" class="btn primary">저장</button></div>`);
  $('#meSave').onclick=async()=>{const button=$('#meSave'),title=$('#meTitle').value.trim(),text=$('#meText').value;if(!title&&!text.trim())return alert('메모 내용을 입력해 주세요.');button.disabled=true;await addDoc(collection(db,'trips',currentTrip.id,'memos'),{title,text,createdBy:user.uid,createdAt:serverTimestamp()});closeModal()};
}
async function cleanDuplicateMemos(){const all=cache.memos||[],seen=new Set(),duplicates=all.filter(x=>{const key=`${x.title||''}\u0000${x.text||''}`;if(seen.has(key))return true;seen.add(key);return false});if(!duplicates.length)return;if(!confirm(`동일하게 반복 저장된 메모 ${duplicates.length}개를 정리할까요?`))return;for(let i=0;i<duplicates.length;i+=400){const batch=writeBatch(db);duplicates.slice(i,i+400).forEach(x=>batch.delete(doc(db,'trips',currentTrip.id,'memos',x.id)));await batch.commit()}}
function renderMemos(){
  const all=cache.memos||[],seen=new Set(),a=all.filter(x=>{const key=`${x.title||''}\u0000${x.text||''}`;if(seen.has(key))return false;seen.add(key);return true}),duplicateCount=all.length-a.length;
  $('#panel-memos').innerHTML=`<div class="section-title"><h2>공동 메모</h2><div class="actions">${duplicateCount?`<button id="cleanMemos" class="btn">중복 ${duplicateCount}개 정리</button>`:''}<button id="addMemo" class="btn primary">+ 메모</button></div></div><div class="list">${a.length?a.map(x=>`<div class="item"><div><h4>${esc(x.title||'메모')}</h4><div>${esc(x.text)}</div></div><div class="actions"><button class="btn small" data-edit="memos:${x.id}">수정</button><button class="btn small" data-del="memos:${x.id}">삭제</button></div></div>`).join(''):'<div class="empty">예약 주의사항, 쇼핑 목록, 꼭 할 일 등을 적어두세요.</div>'}</div>`;
  if(duplicateCount)$('#cleanMemos').onclick=cleanDuplicateMemos;$('#addMemo').onclick=memoForm;bindDeletes();
}
...
function editRecord(sub,id){
  ...
  if(sub==='packing')body=`<div class="field"><label>준비물</label><input id="edText" value="${esc(item.text||'')}"></div><div class="field"><label>챙길 사람 (선택·복수 가능)</label><div class="member-choice-grid">${packingAssigneeChoices(Array.isArray(item.assigneeUids)?item.assigneeUids:[])}</div></div>`;
  ...
  $('#editSave').onclick=async()=>{
    ...
    try{
      ...
      if(sub==='flights')Object.assign(data,{airline:$('#edAirline').value.trim(),flightNo:$('#edFlightNo').value.trim(),date:$('#edDate').value,from:$('#edFrom').value.trim(),depart:$('#edDepart').value,to:$('#edTo').value.trim(),arrive:$('#edArrive').value,booking:$('#edBooking').value.trim(),note:$('#edFlightNote').value.trim()});
      if(sub==='stays')Object.assign(data,{name:$('#edName').value.trim(),checkin:$('#edCheckin').value,checkout:$('#edCheckout').value,address:$('#edAddress').value.trim(),booking:$('#edBooking').value.trim(),note:$('#edStayNote').value.trim()});
      ...
      if(sub==='packing'){const assigneeUids=$('input[name="packAssignee"]:checked').map(x=>x.value),completedByUids=(item.completedByUids||[]).filter(uid=>assigneeUids.includes(uid));Object.assign(data,{text:$('#edText').value.trim(),assigneeUids,completedByUids});}
...
window.addEventListener('online',()=>{if(user&&!currentTrip)listenTrips()});
window.addEventListener('focus',()=>{if(user&&!currentTrip)listenTrips()});
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&user&&!currentTrip)listenTrips()});
