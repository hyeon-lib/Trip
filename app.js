...
async function fxToBase(currency,amount,manualRate=0){
  ...
  try{
    const response=await fetch(`https://api.frankfurter.app/latest?from=${encodeURIComponent(currency)}&to=${encodeURIComponent(currentTrip.baseCurrency)}`);
    if(!response.ok)throw Error('지원하지 않는 통화이거나 환율 서비스가 응답하지 않습니다.');
    const data=await response.json(),rate=Number(data.rates?.[currentTrip.baseCurrency]||0);
    if(!rate)throw Error('선택한 통화의 자동 환율이 없습니다.');
    return {rate,base:numericAmount*rate,source:'auto'};
  }catch(e){
    throw Error(`자동 환율을 불러오지 못했습니다. 수동 환율(1 ${currency} = 몇 ${currentTrip.baseCurrency}인지)을 입력해 주세요.`);
  }
}
function expenseForm(){
  ...
  modal(`<h3>지출 추가</h3>
    ...
    <div id="manualRateWrap" class="field"><label>수동 환율 (선택)</label><input id="exManualRate" type="number" min="0" step="any" placeholder="1 ${foreign} = 몇 ${currentTrip.baseCurrency}"><p class="note">비워두면 자동 환율을 사용합니다. 자동 조회가 실패하면 이 값을 입력해 저장하세요.</p></div>
